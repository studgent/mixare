...
public class RemoteDataHandler extends DataHandler implements DataProcessor {
	...
	public List<Marker> load(String rawData, int taskId, int colour)
		...
		try {
			List<InitialMarkerData> initialMarkerData = iDataHandlerService
	...
	private List<Marker> initializeMarkerData(
			List<InitialMarkerData> initialMarkerData)
			throws PluginNotFoundException, RemoteException {
		List<Marker> markers = new ArrayList<Marker>();
		for (InitialMarkerData i : initialMarkerData) {

			System.out.println(i.getMarkerName());
			Marker marker = PluginLoader.getInstance().getMarkerInstance(
					...
					(Integer) i.getConstr()[7], (String) i.getConstr()[8]);
	...
	private Marker fillExtraMarkerParcelableProperties(Marker marker,
			Map<String, ParcelableProperty> properties) {
		Iterator<Entry<String, ParcelableProperty>> it = properties.entrySet()
				.iterator();
		while (it.hasNext()) {
			Entry<String, ParcelableProperty> pairs = (Entry<String, ParcelableProperty>) it
					.next();
	...
	private Marker fillExtraMarkerPrimitiveProperties(Marker marker,
			Map<String, PrimitiveProperty> properties) {
		Iterator<Entry<String, PrimitiveProperty>> it = properties.entrySet()
				.iterator();
		while (it.hasNext()) {
			Entry<String, PrimitiveProperty> pairs = (Entry<String, PrimitiveProperty>) it
					.next();
			marker.setExtras(pairs.getKey(), pairs.getValue());
		}
		return marker;
